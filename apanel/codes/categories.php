...
if (isset($_POST['submit1'])) {
    ...
    if ($_REQUEST['mode'] == 'add') {
        global $conn;
        $s1_max_orderid = "select max(cat_order) as orderid from tbl_categories";
...
if (!empty($_REQUEST['code']))
    $data = getTable('tbl_categories', "cat_id = '" . $_REQUEST["code"] . "'");

if (isset($_POST['cat_delete']))
    deleteRows("tbl_categories", "cat_id", $_POST['cat_delete']);

if (isset($_POST['cat_status']))
    updateRows("tbl_categories", "cat_status", "cat_id", $_POST['cat_status']);

if (isset($_POST['cat_label']))
    updateRows("tbl_categories", "cat_label", "cat_id", $_POST['cat_label']);

if (isset($_POST)) {
    global $conn;
    $query_rs_pages = "SELECT * FROM tbl_categories ORDER BY cat_order ASC";
    ...
    while ($row_rs_pages = $rs_pages->fetch_array()) {
        if (isset($_REQUEST["orderid" . $row_rs_pages["cat_id"] . ""])) {
            $s35 = "update tbl_categories set cat_order = '" . $_REQUEST["orderid" . $row_rs_pages["cat_id"] . ""] . "' where cat_id = '" . $row_rs_pages["cat_id"] . "' ";
