...
$new_vals['order_data'] = serialize($vals);
...
// validations
if (empty($fullname) || empty($email) || empty($phone_no) || empty($pkg_code) || empty($price)) {
    $output['status'] = FALSE;
...
if ($output['status']) {
    ...
    $admin_data = array(
        ...
        "MSG_BODY" =>  nl2br($_POST['pkg_price'])
    );
    $msg_data = array(
        "Ful Name" => $fullname,
        "Email Address" => $email,
        "Phone Number" => $phone_no,
        "Pakage Code" => $pkg_code,
        "Pakage Price" => $price
    );
    ...
    $pkg_code = "Contact Us : " . stripslashes($_POST["email"]);
    $headers = "MIME-Version: 1.0\r\n";
    $headers .= "Content-type: text/html;charset=utf-8\r\n";
    $headers .= "From: No-Reply <noreply@$domain>" . "\r\n";
    $headers .= "Reply-To: No-Reply <noreply@$domain>" . "\r\n";
    if ($site_driver == "mail") {
        @mail($emailto, $pkg_code, $template, $headers);
        $output['message'] = "Thank You , Your Order Has Been Places Successfully";
    } else {
    ...
    $emailto = $email;
    $pkg_code = $email_tempplate['email_subject'];
    $headers = "MIME-Version: 1.0\r\n";
    $headers .= "Content-type: text/html;charset=utf-8\r\n";
    $headers .= "From: No-Reply <noreply@$domain>" . "\r\n";
    $headers .= "Reply-To: No-Reply <noreply@$domain>" . "\r\n";
